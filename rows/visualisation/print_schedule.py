...
def get_histogram(writer, sheet, utilisation_hist, utilisation_hist_labels):
    ...
    worksheet.insert_chart('L7', chart)
...
def generate_MSExcel(problem_file,input_file,output_file):
    ...
    if not strtobool(carers[0].dropped):
        date_time = datetime.datetime.strptime(visits[0].start_time, '%Y-%b-%d %H:%M:%S')
    ...
    with rows.routing_server.RoutingServer() as session:
        ...
        workbook  = writer.book
        format_double = workbook.add_format({'num_format': '0.000'})
        ...
        for carer in carers:        
            ...
            if not strtobool(carer.dropped):
                ...
                for i in range(0,len(carer.visits)):
                    ...
                    for real_visit in visits_fromDB:
                        if real_visit.visit.key == int(visit.id):
                            ...
                            break
                    if not found_flag:
                        raise ValueError('Visit ' + str(key) + ' not found')
                ...
                time_to_print.replace(hour = int(time_to_print.hour*sign))

                partial_idle_time =  convert_to_seconds(time_to_print)
                opt_utilisation_hist_labels.append(int(carer.sap_number))
                opt_utilisation_hist.append({'service time': partial_work_time, 'travel time': partial_travel_time, 'idle time': partial_idle_time}) 
                ...
                current_row = row_id + len(dict_dataframe_opt['Carer SAP'])
                worksheet_opt.write(current_row + 1,0, "Total travel time for carer: " + convert_to_time_from_seconds(int(partial_travel_time)).strftime('%H:%M:%S') + ", from solution: " + carer.work_travel_time)
                worksheet_opt.write(current_row + 2,0, "Total service time for carer: " + convert_to_time_from_seconds(int(partial_work_time)).strftime('%H:%M:%S') + ", from solution: " + carer.work_service_time)
                worksheet_opt.write(current_row + 3,0, "Total idle time for carer: " + convert_to_time_from_seconds(int(partial_idle_time)).strftime('%H:%M:%S')+ ", from solution: " + carer.work_idle_time)
                
                row_id = current_row + 4
            else:
        ...
        for carer in carers_dict.keys():
            ...
            current_row = row_id + len(dict_dataframe_man['Carer SAP'])
            worksheet_man.write(current_row + 1,0, "Total travel time for carer: " + convert_to_time_from_seconds(int(partial_travel_time)).strftime('%H:%M:%S'))
            worksheet_man.write(current_row + 2,0, "Total service time for carer: " + convert_to_time_from_seconds(int(partial_work_time)).strftime('%H:%M:%S'))
            worksheet_man.write(current_row + 3,0, "Total idle time for carer: " + convert_to_time_from_seconds(int(partial_idle_time)).strftime('%H:%M:%S'))
            
            row_id = current_row + 4


    worksheet_opt.write(0, 0, "ROWS solution for " + date_time.date().strftime("%Y-%m-%d"))
    worksheet_man.write(0, 0, "Human Planner solution for " + date_time.date().strftime("%Y-%m-%d"))
    ...
    worksheet_opt.write(2, 4, "Number of carers: " + str(len(carers)) + ", dropped: " + ', '.join(dropped_carers) )
    worksheet_man.write(2, 4, "Number of carers: " + str(len(carers_dict.keys())) )
    worksheet_opt.write(3, 4, "Number of visits: " + str(sum(len(carer.visits) for carer in carers)) )
    worksheet_man.write(3, 4, "Number of visits: " + str(len(visits_fromDB)) )

    # add histograms
    get_histogram(writer, 'Optimiser', opt_utilisation_hist, opt_utilisation_hist_labels)
    get_histogram(writer, 'Human Planner', man_utilisation_hist, man_utilisation_hist_labels)
