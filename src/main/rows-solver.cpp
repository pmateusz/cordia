...
std::vector<rows::CalendarVisit> FilterVisits(const rows::Problem &problem, const PredicateType &predicate) {
...
public:
    ...
    void Run() override {
        ...
        auto routing_parameters = CreateEngineConfig(FLAGS_maps);
        auto search_parameters = rows::SolverWrapper::CreateSearchParameters();
        search_parameters.set_use_light_propagation(false);
        ...
        try {
            ...
            const auto is_relaxed = [&solver_wrapper, &patched_assignment, &model, this](
                    const rows::CalendarVisit &visit) -> bool {
                static const auto time_dimension_ptr = model->GetMutableDimension(rows::SolverWrapper::TIME_DIMENSION);
