...
TEST(TestVRP, CanSolveRandomProblem) {
    const auto vrp_stops = 25;
    const auto vrp_orders_per_stop = 5;
    const auto vrp_vehicles = 20;
    const auto vrp_use_deterministic_random_seed = false;

    CHECK_LT(0, vrp_stops) << "Specify an instance size greater than 0.";
    CHECK_LT(0, vrp_orders_per_stop) << "Specify an instance size greater than 0.";
    CHECK_LT(0, vrp_vehicles) << "Specify a non-null vehicle fleet size.";
    const int vrp_orders = vrp_stops * vrp_orders_per_stop;
    // Nodes are indexed from 0 to vrp_orders, the starts and ends of the routes
    // are at node 0.
    const RoutingModel::NodeIndex kDepot(0);
    RoutingModel routing(vrp_orders + 1, vrp_vehicles, kDepot);
    RoutingSearchParameters parameters = operations_research::BuildSearchParametersFromFlags();
    ...
    LocationContainer locations(kSpeed, vrp_use_deterministic_random_seed);
    for (int stop = 0; stop <= vrp_stops; ++stop) {
        const int num_orders = stop == 0 ? 1 : vrp_orders_per_stop;
    ...
    ACMRandom randomizer(GetSeed(vrp_use_deterministic_random_seed));
    const int64 kTWDuration = 5 * 3600;
    for (int stop = 0; stop < vrp_stops; ++stop) {
        const int64 start = randomizer.Uniform(kHorizon - kTWDuration);
        for (int stop_order = 0; stop_order < vrp_orders_per_stop;
             ++stop_order) {
            const int order = stop * vrp_orders_per_stop + stop_order + 1;
